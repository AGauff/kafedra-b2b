        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {formats.map((f, i) => (
            <div key={i} className="border border-gray-200 p-8">
              <span className="inline-block bg-gray-100 text-gray-500 text-xs font-bold tracking-widest px-2 py-1 mb-6">{f.tag}</span>
              <h3 className="text-xl font-bold text-black mb-4">{f.title}</h3>
              <ul className="space-y-3">
                {f.items.map((item, j) => (
                  <li key={j} className="text-gray-500 text-base">→ {item}</li>
                ))}
              </ul>
            </div>
          ))}
